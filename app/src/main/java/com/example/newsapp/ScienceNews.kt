...
import com.example.newsapp.databinding.ActivityScienceNewsBinding
import com.example.newsapp.databinding.ActivitySportsNewsBinding
...
class ScienceNews : AppCompatActivity() {
    private val articlesList = mutableListOf<NewsArticle>()
    private lateinit var binding: ActivityScienceNewsBinding

    private lateinit var adapter: NewsAdapterTech

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityScienceNewsBinding.inflate(layoutInflater)
        setContentView(binding.root)

        adapter = NewsAdapterTech(articlesList)
        binding.ScienceRecycleView.adapter = adapter
        binding.ScienceRecycleView.layoutManager = LinearLayoutManager(this)
    ...
    private suspend fun fetchNewsArticles(apiKey: String): String {
        val url = URL("https://api.nytimes.com/svc/topstories/v2/science.json?api-key=$apiKey")
