...
import com.example.newsapp.databinding.ActivityAutomobileNewsBinding
import com.example.newsapp.databinding.ActivityFashionNewsBinding
...
class FashionNews : AppCompatActivity() {
    private val articlesList = mutableListOf<NewsArticle>()
    private lateinit var binding: ActivityFashionNewsBinding

    private lateinit var adapter: NewsAdapterTech

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFashionNewsBinding.inflate(layoutInflater)
        setContentView(binding.root)

        adapter = NewsAdapterTech(articlesList)
        binding.FashionRecycleView.adapter = adapter
        binding.FashionRecycleView.layoutManager = LinearLayoutManager(this)
    ...
    private suspend fun fetchNewsArticles(apiKey: String): String {
        val url = URL("https://api.nytimes.com/svc/topstories/v2/fashion.json?api-key=$apiKey")
