...
class AutomobileNews : AppCompatActivity() {
    private val articlesList = mutableListOf<NewsArticle>()
    private lateinit var binding: ActivityAutomobileNewsBinding

    private lateinit var adapter: NewsAdapterTech

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityAutomobileNewsBinding.inflate(layoutInflater)
        setContentView(binding.root)

        adapter = NewsAdapterTech(articlesList)
        binding.AutomobileRecycleView.adapter = adapter
        binding.AutomobileRecycleView.layoutManager = LinearLayoutManager(this)
    ...
    private suspend fun fetchNewsArticles(apiKey: String): String {
        val url = URL("https://api.nytimes.com/svc/topstories/v2/automobiles.json?api-key=$apiKey")
