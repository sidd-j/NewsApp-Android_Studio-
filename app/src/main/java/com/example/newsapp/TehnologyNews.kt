...
import com.example.newsapp.databinding.ActivityTehnologyNewsBinding
...
class TechnologyNews : AppCompatActivity() {
    private val articlesList = mutableListOf<NewsArticle>()
    private lateinit var binding: ActivityTehnologyNewsBinding
    private lateinit var adapter: NewsAdapterTech

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityTehnologyNewsBinding.inflate(layoutInflater)
        setContentView(binding.root)

        adapter = NewsAdapterTech(articlesList)
        binding.TechRecycleView.adapter = adapter
        binding.TechRecycleView.layoutManager = LinearLayoutManager(this)
